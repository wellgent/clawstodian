...
- name: urgent-check
  interval: 30m
  prompt: |
    Scan for urgent problems and obvious blockers. Reply HEARTBEAT_OK if nothing needs attention.

    Checks (stop at first real signal):
    1. Failed cron runs in the last hour - `openclaw cron list` then inspect recent run logs for any disabled-on-error or repeated failures.
    2. Unexpected working-tree state - mid-rebase, detached HEAD, unresolved conflicts.
    3. Any in-progress automation that looks stuck (stale lock files, pid files pointing at dead processes).

    If you find something urgent and the fix is reversible and obvious, do it and report one line. If not obvious, surface one short alert with: what was observed, likely cause, one or two resolution paths. Do not start a larger investigation; that is for workspace-sweep.
...
- name: workspace-sweep
  ...
  prompt: |
    ...
    3. Verify heartbeat config still matches the recommended stance (`isolatedSession`, `lightContext`, `target`, `activeHours`).
...
- Never invent projects or workstreams. The maintainer works inside what exists.
