...
- name: tend-para-extract
  interval: 24h
  prompt: "Read clawstodian/routines/para-extract.md. Review new memory/runs/para-extract/*.md reports since the previous tick - were entities updated / created as expected, any ambiguous items surfaced. Then detect the routine's queue per its Queue definition (status: sealed AND para_status: pending). Enable or disable its cron to match. Contribute findings to this tick's channel post."
- name: reflect
  interval: 24h
  prompt: "Daily reflection. Two scans. (1) Routines: new memory/runs/*/*.md across sessions-capture, daily-seal, para-extract, workspace-clean, git-clean, para-align, health-check since the previous reflect - aggregate operator-surfaced items, note repeats now stale. (2) Heartbeat dynamics: memory/heartbeat-trace.md for the last 24h - did tasks fire on their expected cadences, did toggles happen when queues formed, any silent gaps or misfires in the orchestrator itself. Write a 2-4 sentence narrative directly to the operator in this DM, naming stale attention items AND any orchestrator-dynamics issues worth surfacing. Echo the narrative in this tick's channel post."
