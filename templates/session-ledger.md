...
The ledger is NOT a log, and NOT a dashboard for humans. It is a compact authoritative state file that the `daily-note` cron and the `backfill-sessions` cron both read and update. Operators rarely need to touch it by hand.
...
- **New session seen in `sessions_list` and outside the steady-state window** - `backfill-sessions` creates an entry, processes the entire transcript, and marks `status: done` if the session is dormant (no activity in N days) or leaves `status: active` otherwise.
- **New session seen in `sessions_list` inside the steady-state window** - `daily-note` creates an entry and processes whatever has been written so far, just as it would for a returning active session.
- **Known active session with new JSONL lines** - `daily-note` reads from `lines_captured` to end, appends to the appropriate daily notes, advances the cursor.
...
<!-- One H2 section per session, sorted by first_seen ascending. Append new
     ...
     existing sections. The file is read in full by daily-note and
     backfill-sessions on each run. -->
...
- The `daily-note` and `backfill-sessions` routines are the only writers. The heartbeat reads but does not write.
