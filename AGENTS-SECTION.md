...
- **Never invent projects.** The maintainer works inside what the operator has started. New initiatives come from the operator, not the maintainer.
...
- **Approval gate.** Ask before committing any change that touches more than 10 files in a single logical commit. Ask before committing to `.openclaw/` or other config directories. Ask before committing anything that looks like it could contain secrets (`.env*`, keys, tokens).
...
- **What NOT to do.** No force-push. No `git reset --hard` without operator confirmation. No `git clean -f`. No amending published commits. No committing to branches the operator has not opted into. No emoji in commit messages unless the workspace style already uses them.
...
- **Execution steps.**
  ...
  3. Check heartbeat config matches the recommended stance (`isolatedSession: true`, `lightContext: true`, `target: "none"`, `activeHours` set).
...
Recommended config for workspaces running clawstodian:
...
`isolatedSession: true` + `lightContext: true` drops each heartbeat tick from roughly 100K tokens to 2-5K. That is a deliberate choice: cross-tick memory lives in the workspace, not in session history. Every tick reads the state it needs from files, acts, writes its observations back to files, and forgets.

## Reference files

- `memory/para-structure.md` - entity types, frontmatter, naming.
- `memory/daily-note-structure.md` - daily note format.
- `memory/crons.md` - cron job dashboard.
- `MEMORY.md` - curated workspace dashboard.
