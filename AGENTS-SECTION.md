...
- **Authority.** Stage specific files, commit, and push. Edit `.gitignore`. Read `git log`, `git status`, `git diff`. Never force-push, never rewrite history, never push to branches the operator has not opted into.
...
- **Execution steps.**
  ...
  3. For each group: stage files by exact path (never `git add -A` or `git add .`). Commit message format: `<topic>: <short description>` - lowercase, specific, meaningful. Never `changes`, `updates`, `misc`. Common topics: `memory:` (daily notes), `para:` (PARA entities), `config:` (settings), `fix:` (bug fixes), `content:` (non-memory content), `insight:` (durable insights), `guide:` (guides under `resources/`), `skill:` (`.agents/skills/`), `ops:` (operations or infrastructure). Topic reflects what changed, not which repo. Workspaces can extend with their own topics. For significant changes, add a body: `-m "summary" -m "- bullet one" -m "- bullet two"`. Commit, then push immediately - unpushed commits are invisible to other sessions. If push fails (network, auth, protected branch), surface the error; do not keep piling commits on top.
