...
- Recommended heartbeat gateway config switched from isolated-session to a persistent named session (`session:clawstodian-maintainer`) with enforced compaction (`session.maintenance` block, `maxEntries: 300`). Conversation history persists across ticks so the agent and operator carry a continuous maintenance thread. Per-tick cost rises from ~5K to ~25-50K tokens in exchange for collaborative continuity. `docs/heartbeat-config.md` has the full rationale.
