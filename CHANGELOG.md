...
- `routines/backfill-sessions.md` (new) - heartbeat-toggled burst that processes one historical session per firing: picks the oldest `sessions_list` row without a ledger entry, classifies it, reads the full transcript, buckets by date, applies to active-date notes and surfaces bleed-over for sealed-date buckets. Self-disables when the ledger catches up to `sessions_list`.
...
- Seven-routine catalog (four programs):
  - `daily-note` (always-on, every 30m) -> `daily-notes` program, ingest recent activity (90m window).
  - `backfill-sessions` (burst) -> `daily-notes` program, ingest one historical session per firing.
...
- `templates/AGENTS.md` (new) carries the workspace charter with two catalogs: four programs (primary, domain authorities) and seven routines (scheduled invocations appendix). Replaces the earlier `AGENTS-SECTION.md`.
...
- `templates/crons.md` lists all seven routines with the program/behavior each invokes.
- `programs/daily-notes.md` rewritten around the session-ledger cursor model: explicit prerequisites (`tools.sessions.visibility: "all"`), classification-before-tracking (cron / hook / subagent / delivery-only sessions filtered and recorded as `skipped`), per-session line-count cursor, sparse date detection from JSONL timestamps, past-date bleed-over surfacing for sealed notes. Three behaviors instead of two: "Ingest recent activity", "Ingest a historical session", "Seal a past-day note". The first two are the capture loop ops-daily got right, implemented without its overhead (no `capture-state.json` sidecar, no per-version schema migrations, no multiple extraction scripts).
- `routines/daily-note.md` refocused to steady-state only (90m `activeMinutes` window); historical catch-up moves to `backfill-sessions`. Run report gains session-level counters (`sessions active/captured/new-skipped`, `bleed`).
- `templates/HEARTBEAT.md` status sweep extended: ledger-gap detection toggles `backfill-sessions`; session-visibility drift surfaces as a health anomaly; tick trace line gains a `backfill=<0|1>` field; channel summary line reports `queues: backfill= seal= extract=`.
- `templates/AGENTS.md` catalog now lists seven routines and names `memory/session-ledger.md` as workspace state.
