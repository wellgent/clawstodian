...
- Seven-routine catalog (four programs):
  - `daily-note` (always-on, every 30m) -> `daily-notes` program, ingest recent activity (6h window).
...
- `routines/daily-note.md` refocused to steady-state only (6h `activeMinutes` window - wide enough to absorb a single-digit-hour gateway outage without a silent capture gap); historical catch-up moves to `backfill-sessions`. Run report gains session-level counters (`sessions active/captured/new-skipped`, `bleed`).
