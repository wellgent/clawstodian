...
- `scripts/` - deterministic helpers that agents invoke by path rather than re-implementing in prompt. Currently: `scan-sessions.py` (interactive-session queue source shared by the heartbeat orchestrator and `sessions-capture`).
